      ...
      <div className="sticky top-0 z-10 bg-slate-50 border-b border-slate-200 px-4 py-4 lg:px-8">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-slate-400 text-xs">{orders.length} total · {orders.filter(o => o.status !== 'delivered').length} active</p>
          </div>
          <Link href="/orders/new"
            className="flex items-center gap-1.5 bg-blue-600 text-white text-sm font-semibold px-3 py-2 rounded-xl shadow-sm hover:bg-blue-700 transition">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium border transition ${
              statusFilter ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'
            }`}>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            <button onClick={() => { setStatusFilter(''); setShowFilters(false); }}
              className={`px-3 py-1 rounded-full text-xs font-medium border transition ${
                !statusFilter ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500 border-slate-200'
              }`}>All</button>
            {ORDER_STATUSES.map(s => (
              <button key={s} onClick={() => { setStatusFilter(s); setShowFilters(false); }}
                className={`px-3 py-1 rounded-full text-xs font-medium border transition ${
                  statusFilter === s ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500 border-slate-200'
                }`}>
                    ...
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          ...
                          {order.is_urgent && (
                            <span className="bg-red-100 text-red-600 text-xs font-bold px-1.5 py-0.5 rounded-full">URGENT</span>
                          )}
                        </div>
                      </div>
                    </div>
