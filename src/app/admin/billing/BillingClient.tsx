...
import { FileText, Download, TrendingUp, CheckCircle2, Clock, AlertCircle, Loader2, DollarSign, Search } from 'lucide-react';
...
export default function BillingClient({ invoices: initialInvoices, profile }: BillingClientProps) {
  ...
  const filtered = invoices.filter(i => {
    ...
    return i.invoice_number.toLowerCase().includes(q) ||
      (i.profiles as Record<string, string> | undefined)?.shop_name?.toLowerCase().includes(q) ||
      (i.orders as Record<string, string> | undefined)?.customer_name?.toLowerCase().includes(q);
  });
  ...
  const handleMarkPaid = async (invoice: Invoice) => {
    ...
    const { error } = await supabase
      .from('invoices')
      .update({ status: 'paid', paid_amount: invoice.total_amount, payment_date: new Date().toISOString() })
      .eq('id', invoice.id);
    ...
    if (error) {
      ...
      setInvoices(prev => prev.map(i => i.id === invoice.id
        ? { ...i, status: 'paid', paid_amount: i.total_amount }
        : i
      ));
  ...
  const handleDownload = async (invoice: Invoice) => {
    if (!invoice.orders) { toast.error('Order data missing'); return; }
  ...
  const StatusPill = ({ status }: { status: Invoice['status'] }) => {
    const map = { paid: 'bg-green-100 text-green-700', unpaid: 'bg-red-100 text-red-700', partial: 'bg-amber-100 text-amber-700' };
    return (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {(invoice.profiles as Record<string, string> | undefined)?.shop_name || 'Unknown'}
                      </p>
                      {invoice.orders && (
                        <p className="text-xs text-slate-400">
                          {(invoice.orders as Record<string, string>).order_number} · {(invoice.orders as Record<string, string>).customer_name}
                        </p>
                      )}
                  ...
                  <div className="flex-shrink-0 text-right space-y-2">
                    ...
                    <div className="flex items-center gap-1.5 justify-end">
                      {invoice.status !== 'paid' && (
                        <button onClick={() => handleMarkPaid(invoice)} disabled={updating === invoice.id}
